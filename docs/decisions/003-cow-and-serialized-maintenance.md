...
Silver의 delete 파일 수·크기와 Athena 조회 시간을 관측한다. 읽기 비용이 과도하면 compaction 임계값을 조정하거나 COW 복귀를 검토한다. 30일 이전 상태로 time travel이나 rollback이 필요하면 snapshot 보존 기간을 늘린다. 원본 기반 백필 가능 범위는 snapshot이 아니라 Bronze 보존 기간에 달려 있다.
...
- [Iceberg 유지보수 DAG](../../airflow/dags/iceberg_maintenance_dag.py)
